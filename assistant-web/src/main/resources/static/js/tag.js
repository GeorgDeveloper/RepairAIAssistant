...
function formatTime(value) {
    if (!value) return '';
    // Если это строка формата "HH:MM:SS" или "HH:MM"
...
function formatDetailsRow(d) {
    var rows = Object.entries(d || {}).map(function(entry){
        ...
        if (key.includes('date') || key === 'created_at' || key === 'start_bd_t1' || key === 'start_maint_t2' || key === 'stop_maint_t3' || key === 'stop_bd_t4') {
            if (key === 'production_day' || key === 'date') {
        ...
        // Форматируем время
        if (key.includes('time') || key === 'machine_downtime' || key === 'ttr' || key === 't2_minus_t1') {
...
$(document).ready(function () {
    ...
    var table = $('#tagTable').DataTable({
        ajax: { 
            url: '/dashboard/tag-maintenance-records', 
            dataSrc: function(json) {
                // Фильтруем по текущему году по умолчанию
                const currentYear = new Date().getFullYear();
                return json.filter(row => {
                    const dt = new Date(row.start_bd_t1 || row.production_day || row.date);
        ...
        columns: [
            ...
            { data: 'additional_kit', title: 'Доп. комплект', width: '150px' },
            { data: 'description', title: 'Описание', width: '200px' },
            { data: 'code', title: 'Код', width: '100px' },
            { data: 'hp_bd', title: 'HP BD', width: '80px' },
            { data: 'start_bd_t1', title: 'Начало BD T1', width: '140px', render: function(d){ return formatDateTime(d); } },
            { data: 'start_maint_t2', title: 'Начало ремонта T2', width: '140px', render: function(d){ return formatDateTime(d); } },
            { data: 'stop_maint_t3', title: 'Окончание ремонта T3', width: '140px', render: function(d){ return formatDateTime(d); } },
            { data: 'stop_bd_t4', title: 'Окончание BD T4', width: '140px', render: function(d){ return formatDateTime(d); } },
            { data: 'machine_downtime', title: 'Время простоя', width: '120px', render: function(d){ return formatTime(d); } },
            { data: 'ttr', title: 'TTR', width: '100px', render: function(d){ return formatTime(d); } },
            { data: 't2_minus_t1', title: 'T2-T1', width: '100px', render: function(d){ return formatTime(d); } },
            ...
            { data: 'failure_type', title: 'Тип неисправности', width: '150px' },
            { data: 'area', title: 'Участок', width: '120px' },
            { data: 'created_at', title: 'Создано', width: '140px', render: function(d){ return formatDateTime(d); } },
            { data: 'date', title: 'Дата', width: '100px' },
            { data: 'week_number', title: 'Неделя', width: '90px' },
            { data: 'month_name', title: 'Месяц', width: '120px' },
            { data: 'shift', title: 'Смена', width: '100px' },
            { data: 'staff', title: 'Персонал', width: '150px' },
            { data: 'crew', title: 'Бригада', width: '120px' },
            { data: 'crew_de_facto', title: 'Бригада де-факто', width: '150px' },
    ...
    function populateFilters() {
        ...
        const years = Array.from(new Set(data.map(r => { 
            const d = new Date(r.start_bd_t1 || r.production_day || r.date); 
    ...
    $('#tagTable tbody').on('click', 'td.details-control', function () {
