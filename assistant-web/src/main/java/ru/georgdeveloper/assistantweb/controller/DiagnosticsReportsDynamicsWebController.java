...
public class DiagnosticsReportsDynamicsWebController {
    ...
    public DiagnosticsReportsDynamicsWebController(RestTemplate restTemplate) {
    ...
    public ResponseEntity<List<Map<String, Object>>> getDynamicsData(
        ...
        try {
            StringBuilder url = new StringBuilder(coreServiceUrl + "/api/diagnostics-reports-dynamics/data?");
    ...
    public ResponseEntity<Map<String, Object>> getSummary(
            ...
            @RequestParam(required = false) String diagnosticsType) {
        try {
            StringBuilder url = new StringBuilder(coreServiceUrl + "/api/diagnostics-reports-dynamics/summary?");
            if (year != null) url.append("year=").append(year).append("&");
            if (month != null) url.append("month=").append(month).append("&");
            if (area != null && !area.isEmpty()) url.append("area=").append(java.net.URLEncoder.encode(area, java.nio.charset.StandardCharsets.UTF_8)).append("&");
            if (equipment != null && !equipment.isEmpty()) url.append("equipment=").append(java.net.URLEncoder.encode(equipment, java.nio.charset.StandardCharsets.UTF_8)).append("&");
    ...
    public ResponseEntity<List<String>> getDiagnosticsTypes() {
        try {
            List<String> types = (List<String>) (List<?>) 
                    restTemplate.getForObject(coreServiceUrl + "/api/diagnostics-reports-dynamics/diagnostics-types", List.class);
